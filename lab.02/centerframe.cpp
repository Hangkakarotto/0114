...
//   菱形按钮
   ...
   //图片按钮
   QPixmap pixmap(p.size());    //图标临时保存在pixmap中,大小同p
   QPainter paint(&pixmap);
   QImage image(":/picture/resource/icon1.jpg");     //QImage 读取资源图片
   QRect targetRect(0,0,p.size().width(),p.size().height());
   QRect sourceRect =image.rect();
   paint.drawImage(targetRect,image,sourceRect);

   btnGraph= new QPushButton(group);     //btnGraph加入组中
   btnGraph->setIcon(QIcon(pixmap));
   btnGraph->setCheckable(true);             //设置 check 属性
   btnGraph->setIconSize(p.size());
   btnGraph->setToolTip("绘制图片文件");
   connect(btnGraph,&QPushButton::clicked,this,&CenterFrame::on_btnGraphClicked);
    ...
    gridLayout->addWidget(btnGraph,3,0);
...
//graph被按下
 void CenterFrame::on_btnGraphClicked()
 {
  if(btnGraph->isChecked()){
    drawWidget->grap() ;           //检查,被按下则画图
    updateButtonStatus();
  }else{
    drawWidget->setShapeType(ST::None);
  }
